...
- 2026-08-06, ux, **the run page read summary keys that only a dry run wrote.** The cards read `new`, `updated` and `unchanged`; a live run wrote `planned_new`, `transferred` and no `unchanged` at all. So every live run showed New 0, Updated 0 and Unchanged 0 however many files it moved, and the byte line read "0 B would transfer" in the past tense. Nothing failed, because `summary.get(key, 0)` cannot tell a missing key from a real zero. Two writers filling one reader's dictionary need a test that compares the key sets, which `test_the_run_page_reads_keys_that_a_live_run_actually_writes` now is. Defaulting a lookup to zero is the thing that hid it.
- 2026-08-06, rclone, a live run **can** tell a new file from an updated one, and it is the only place that distinction exists: verified against 1.74.4, `Copied (new)` versus `Copied (replaced existing)`, with `Multi-thread Copied (new)` over the cutoff. A dry run reports `skipped: "copy"` for both, which is why the planner needs its separate presence pass. `PlannedOperation.replaced` is None for the dry run case rather than False, so "not known" and "was not a replacement" stay distinct.
- 2026-08-06, jinja, **there are no comments inside an expression.** A `{# ... #}` placed within a `{% set xs = [...] %}` list raises `unexpected char '#'` at render time, not at startup, so it 500s every page that extends the template while every module still imports cleanly. Put the note above the block.
...
- 2026-08-07, ux, **the dashboard never refreshed itself, so a finished run stayed under "Running now" until someone pressed reload.** It is a snapshot of state that changes with nobody touching the page. The activity strip's poll was not a substitute: it updates its own figures only, and `activity.active()` is the in-process recorder, which is empty for a run between stats events or after a restart, so it cannot answer "is this job still running". The dashboard polls the database instead, with the interval rendered by the server on each swap: 5s while anything runs, 30s when nothing does. Verified in a browser rather than by reading attributes: two polls in seventy seconds, three swaps, and a probe attribute set beforehand was gone afterwards, which is what proves the element was actually replaced. The swapped-in element keeps its own `hx-get`, so the poll survives replacing itself.
- 2026-08-07, htmx, **a poll whose response redirects to the login page silently blanks the element it targets.** htmx follows the redirect, `hx-select` finds nothing in the login page, and it swaps in nothing: an expired session turns a self-refreshing page into an empty one. A `htmx:beforeSwap` handler cancels the swap and reloads instead, so the reader gets the login form.

- 2026-08-10, planning, **a dry run of a bidirectional job was refused outright**, and the message promised a milestone that had shipped three milestones earlier. `planner.engine_for` returned `RcloneEngine` for every job, and `RcloneEngine.plan` raises for bidirectional. So the one direction that can damage both copies was the one that could not be previewed, which is the opposite of where a preview is most needed. `BisyncEngine.plan` now runs `bisync --dry-run` and reconciles it; `engine_for` branches on direction.
- 2026-08-10, rclone, **verify against the flags the application passes, not the ones convenient to type by hand.** `parse_planned_changes` was written against plain text captured from a manual `rclone bisync --dry-run`, and its unit tests passed. `build_bisync_command` passes `--use-json-log`, so what the application actually receives is JSON with the line inside `msg`. The end-anchored regex then captured `","source":"bisync/deltas.go:259"}` as part of every filename. The unit tests were green and the integration test failed, which is the only reason it was caught. A captured sample is only evidence if it was captured the way the code will see it.
- 2026-08-10, rclone, bisync's per-file diff wording, verified against 1.74.4: `File is new`, `File was deleted`, and `File changed: size (larger), time (newer)` with the varying attributes spelled out. The `Queue copy to PathN` and `Queue delete` lines that follow repeat the same changes as **absolute** destination paths; the diff lines carry relative ones. Only the diff lines are parsed, because two sources for one fact drift apart.
- 2026-08-10, licensing, the repository was unlicensed until now, which is more restrictive than any licence: nobody had permission to use the published image. Now MIT. lftp is GPL-3 (verified from `/usr/share/doc/lftp/copyright`) and rclone is MIT (verified from its `COPYING`); both are driven as subprocesses rather than linked, so neither constrains HiveSync's own licence.
...
- 2026-08-11, ux, **every new file a one way live run copied was recorded as "updated", since M3.** The summary cards counted through `observed.created`, which is rclone's own `Copied (new)` wording, while the per-file rows were derived from whether the pre-flight plan had mentioned the path: `ChangeAction.new if op.path not in planned_paths else updated`. `plan.changes` holds every planned change of every kind, so "the plan saw this file" was read as "this file was an update", and the only rows that could ever come out new were files that appeared *after* planning. Exactly backwards. Reported from production on a 9.3 GB file: one screen showed **New 1** in the cards above a table whose only row said **updated**. A run that contradicts itself in two places is the tell that one fact has two writers, which is the fourth time this shape has appeared here. Both now read `op.replaced`, which is what `_record_bisync` already did; the newer code was the correct one.
- 2026-08-11, testing, **the integration tests queried these rows three times and never looked at `action`.** They collected `{c.path for c in ...}` and asserted on the set of paths, so a row with the wrong label was invisible to a suite running the real binary against real fixtures. Asserting that a change was *recorded* is not asserting that it was recorded *as the right thing*. `test_a_new_file_is_labelled_new_and_an_overwrite_updated` now checks the label and that the cards agree with the table, and it was confirmed to fail against the old rule rather than merely pass against the new one.

- 2026-08-11, auth, **`HIVESYNC_API_TOKEN` is rejected on an unclaimed instance, and the 401 does not say why.** The token maps to the bootstrap admin, so with no user row there is nobody for it to authenticate as. That is correct rather than a hole, but it is confusing in exactly the case it will be met: a fresh container started with a token and no `HIVESYNC_ADMIN_PASSWORD`, where a scrape or a script gets a flat 401 while `/api/health` answers fine. Set the admin password, or complete the wizard first.
- 2026-08-11, docker, the application listens on **8080**, not 8000. Recorded because a hand-written `docker run -p ...:8000` produces a container that reports healthy, logs nothing wrong, and refuses every connection: the healthcheck runs inside and uses the right port. `docker compose` gets this right, so it only bites an ad hoc run.
...
- 2026-08-10, tooling, **a scripted `str.replace(old, new, 1)` anchored on a non-unique line silently edits the wrong site.** `deltas = bisync.parse_deltas(text)` appears twice in `runner.py`, once after the run and once in the pre-flight. The delete brake approval was inserted at the first, where `run` and `deltas` are also in scope, so it compiled, typechecked and passed every existing test while doing nothing: every approved run was refused exactly as before, and the early return it added would have skipped `_record_bisync` on a run that succeeded. Anchor on something unique, or read the result back before believing an edit landed where it was aimed.
...
- 2026-08-10, rclone, **the quiet period now applies to every run, not only continuous ones.** `--min-age` was gated on `job.continuous`, so a scheduled job had no protection against a download client writing into its source, which is the case the setting exists for. It is now in the planning phases too: without that a dry run listed a file the live run then skipped, and the two disagreed about the same tree. bisync passes it as well, since it writes to both sides.
- 2026-08-10, design, **it defaults to off, and that was measured rather than reasoned about.** A default of 30 seconds looked protective until ten integration tests failed with `planned_new == 0`: the setting applies to a manual run too, so creating files and pressing Run copies nothing. The original code's comment was half right, in that a non-zero value is surprising for a run someone just asked for, and half wrong, in that restricting it to continuous mode removed it from the case that needed it. Existing non-continuous jobs are migrated to 0 so nothing changed silently.
- 2026-08-10, design, the quiet period default lived in **four** places: the column, the schema, the form handler and the template. Three were changed and the handler's hardcoded `30` was not, so a job created without the field got a quiet period nobody chose and silently copied nothing. Caught by the README walkthrough test, which is the only one that posts the form the way a reader would. The handler derives it from the schema now, and a test asserts all four agree.
- 2026-08-10, ux, the "Downloads in progress" preset is the other half of the answer. `--min-age` watches modification time, so it catches a file still growing but not a client that writes the whole file and rewrites it in place. Excluding the temporary name (`.part`, `.!qB`, `.crdownload`, `.partial`, `.filepart`, and the `incomplete/` and `_UNPACK_*/` directories) is sturdier, because the rename at the end is atomic.

- 2026-08-10, design, **one-click restore of archived deletions**, answering SPEC open question 3. The blocker was not the UI: the archive directory carries the run stamp computed as the run starts, so nothing could find it again, and it was only ever emitted as a status line. `JobRun.archive_dirs` records it per side, **without** the remote prefix, because a run's `hs_src`/`hs_dst` aliases belong to a process that has ended and a stored spec would name a remote that no longer exists.
- 2026-08-10, rclone, **`--ignore-existing` is the safety property, and a skip logs nothing at all.** Verified against 1.74.4: a copy logs `Copied (new) to: <name>`, and a skipped one is silent even at `-v`. The first version of the restore reporter looked for a "Skipped copy as --ignore-existing" message that does not exist, which would have reported every restore as successful including the ones that did not happen. Detect on the positive signal, and match `"Copied" in` rather than a prefix, because a large file logs `Multi-thread Copied`.
- 2026-08-10, design, restore **lists the archive** rather than reading the run's change rows. The rows record what the run archived; the directory records what is still there, and retention or a person may have removed some of it in between. Offering a restore for a file that is gone fails at the moment someone needs it to work. It also means a path arriving in a form is matched against the listing rather than used to build a destination, so a request cannot turn into an arbitrary write.

- 2026-08-10, rclone, **a per-file speed exists only while the file is in flight, and `--stats 5s` means most files are never sampled.** The `transferring` array is the only place it appears, and a file that starts and finishes inside one interval is never in it: a 64MB local copy recorded nothing at all, which is what the first version of the history test proved. Peak speed is therefore recorded where it can be measured and left NULL otherwise, and the screen says so, because a blank column reads as broken unless it explains itself. The stats interval was deliberately not shortened to improve coverage: it feeds the dashboard and is written to every run log, so tightening it multiplies log volume for every sync as a side effect of a history column.
- 2026-08-10, rclone, the stats `name` and the copy line's `object` are the same relative path, verified against 1.74.4 with a nested file: both report `deep/nested/big.bin`. That is what lets a peak speed be matched to the change row it belongs to.
- 2026-08-10, design, the history screen sorts by a key matched against a fixed map rather than by the column name in the query string, so a name arriving in a URL can never reach an ORDER BY. Searching covers the file path and the job name only: matching a size or a speed as text finds coincidences rather than files.
